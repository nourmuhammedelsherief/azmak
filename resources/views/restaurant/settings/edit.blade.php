            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    ...
                    <h3> @lang('messages.branch') :
                        ({{app()->getLocale() == 'ar' ?$setting->branch->name_ar :$setting->branch->name_en}})</h3>
                ...
                <!-- general form elements -->
                    <div class="card card-primary">
                        ...
                        <form role="form" action="{{route('restaurant_setting.update' , $setting->id)}}" method="post"
                            ...
                            <div class="card-body">
                                <div id="hidden_pre"
                                     style="display: {{in_array($setting->order_type , ['previous' , 'whatsapp' , 'easymenu']) ? 'block': 'none'}};">
                                    <div class="form-group">
                                        <span style="font-size:20px"> <b>(1)</b> </span>
                                        <label class="control-label"> @lang('messages.delivery') </label>
                                        <input name="delivery" type="radio" onclick="javascript:yesNoPreDeliveryCheck()"
                                               value="true" {{$setting->delivery == 'true' ? 'checked': ''}}
                                        ...
                                        <input name="delivery" id="preDeliveryCheck"
                                               ...
                                               {{$setting->delivery == 'false' ? 'checked': ''}}
                                    ...
                                    <div id="ifYesPreDeliveryCheck"
                                         style="display:{{($setting->order_type == 'delivery' or $setting->delivery == 'true') ? 'block': 'none'}}">
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <label class="control-label"> الحد الأدني للتوصيل </label>
                                            <input name="minimum_delivery" type="radio"
                                                   onclick="javascript:minimum_deliveryF()" id="minimum_delivery" value="true"
                                                   {{$setting->minimum_delivery == 'true' ? 'checked': ''}}
                                                   placeholder="@lang('messages.delivery')"> @lang('messages.yes')
                                            <input name="minimum_delivery"
                                                   onclick="javascript:minimum_deliveryF()" type="radio" value="false"
                                                   {{$setting->minimum_delivery == 'false' ? 'checked': ''}}
                                                   placeholder="@lang('messages.delivery')"> @lang('messages.no')
                                            @if ($errors->has('minimum_delivery'))
                                                <span class="help-block">
                                            <strong
                                                    style="color: red;">{{ $errors->first('minimum_delivery') }}
                                            </strong>
                                        </span>
                                            @endif
                                        </div>
                                        <div id="minimum_delivery_div" style="display: {{$setting->minimum_delivery == 'true' ? 'block' : 'none'}}">
                                            <div class="form-group">
                                                <label class="control-label"> قيمة الحد ألادني للتوصيل </label>
                                                <div class="row">
                                                    <div class="col-sm-10">
                                                        <input name="minimum_delivery_value" type="number"
                                                               class="form-control"
                                                               value="{{$setting->minimum_delivery_value}}"
                                                               placeholder="قيمة الحد ألادني للتوصيل">
                                                    </div>
                                                    <div class="col-sm-2">
                                                        {{app()->getLocale() == 'ar' ? \Illuminate\Support\Facades\Auth::guard('restaurant')->user()->country->currency_ar : \Illuminate\Support\Facades\Auth::guard('restaurant')->user()->country->currency_en}}
                                                    </div>
                                                ...
                                                @if ($errors->has('minimum_delivery_value'))
                                                    <span class="help-block">
                                            <strong style="color: red;">{{ $errors->first('minimum_delivery_value') }}</strong>
                                        </span>
                                                @endif
                                            </div>
                                        </div>
                                        <a class="btn btn-warning" href="{{route('restaurant_setting_range.index' , $setting->id)}}">
                                                أضافة نطاقات توصيل اخري
                                            </a>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input name="takeaway" type="radio" value="true"
                                               onclick="javascript:yesNoPreTakeawayCheck()"
                                               {{$setting->takeaway == 'true' ? 'checked': ''}}
                                               placeholder="@lang('messages.takeaway')"> @lang('messages.yes')
                                        <input name="takeaway" type="radio" value="false" id="preTakeawayCheck"
                                               onclick="javascript:yesNoPreTakeawayCheck()"
                                               {{$setting->takeaway == 'false' ? 'checked': ''}}
                                    ...
                                    <div id="ifYesPreTakeawayCheck"
                                         style="display:{{$setting->takeaway == 'true' ? 'block' : 'none'}}">
                                        <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input name="previous" type="radio" value="true"
                                               {{$setting->previous == 'true' ? 'checked': ''}}
                                               id="previousYes" name="previous"
                                               onclick="javascript:previousYesNoCheck();"> @lang('messages.yes')
                                        <input name="previous" type="radio" value="false"
                                               {{$setting->previous == 'false' ? 'checked': ''}}
                                    ...
                                    <div id="previous_periods"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="previous_order_type" class="form-control">
                                                <option disabled
                                                        selected> @lang('messages.previous_order_type') </option>
                                ...
                                <div id="IfYesTableCheck"
                                     style="display: {{$setting->table == 'true' ? 'block':'none' }}">
                                    <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <input name="receipt_payment" type="radio"
                                           {{$setting->receipt_payment == 'true' ? 'checked' : ''}} value="true"> @lang('messages.yes')
                                    <input name="receipt_payment" type="radio"
                                           {{$setting->receipt_payment == 'false' ? 'checked' : ''}} value="false"> @lang('messages.no')
                                ...
                                @if($setting->order_type == 'whatsapp')
                                    <div class="form-group bank-transfer">
                                        ...
                                        <input name="bank_transfer"
                                               {{$setting->bank_transfer == 'true' ? 'checked' : ''}} type="radio"
                                               value="true"> @lang('messages.yes')
                                        <input name="bank_transfer"
                                               {{$setting->bank_transfer == 'false' ? 'checked' : ''}} type="radio"
                                ...
                                <div class="form-group">
                                    ...
                                    <input name="online_payment"
                                           {{$setting->online_payment == 'true' ? 'checked' : ''}} onclick="javascript:yesnoCheck();"
                                           id="noCheck"
                                           type="radio" value="true"> @lang('messages.yes')
                                    <input name="online_payment"
                                           {{$setting->online_payment == 'false' ? 'checked' : ''}} onclick="javascript:yesnoCheck();"
                                           id="yesCheck"
                                ...
                                <div id="online_token"
                                     style="display: {{($setting->payment_company == 'tap' || $setting->payment_company == 'myFatoourah') ? 'block':'none'}}">
                                    <div class="form-group">
                                ...
                                <div id="express_keys"
                                     style="display: {{$setting->payment_company == 'express' ? 'block' : 'none'}}">
                                    <div class="form-group">
...
<script type="text/javascript">
    ...
    function minimum_deliveryF() {
        if (document.getElementById('minimum_delivery').checked) {
            document.getElementById('minimum_delivery_div').style.display = 'block';
        } else {
            document.getElementById('minimum_delivery_div').style.display = 'none';
        }
    }
