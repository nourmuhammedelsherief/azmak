            ...
            <div class="col-12">
                <h3>
                    ...
                    <a href="{{ route('restaurant.waiter.orders.index') }}?status=pending"
                        class="btn btn-{{ request('status') == 'pending' ? 'primary' : 'info' }}  ">{{ trans('dashboard.pending') }} ({{$restaurant->waiterOrders()->where('status' , 'pending')->count()}})</a>

                    <a href="{{ route('restaurant.waiter.orders.index') }}?status=in_progress"
                        class="btn btn-{{ request('status') == 'in_progress' ? 'primary' : 'info' }}  ">{{ trans('dashboard.in_progress') }} ({{$restaurant->waiterOrders()->where('status' , 'in_progress')->count()}})</a>

                    <a href="{{ route('restaurant.waiter.orders.index') }}?status=completed"
                        class="btn btn-{{ request('status') == 'completed' ? 'primary' : 'info' }}  ">{{ trans('dashboard.completed') }} ({{$restaurant->waiterOrders()->where('status' , 'completed')->count()}})</a>

                    <a href="{{ route('restaurant.waiter.orders.index') }}?status=canceled"
                        class="btn btn-{{ request('status') == 'canceled' ? 'primary' : 'info' }}  ">{{ trans('dashboard.canceled') }} ({{$restaurant->waiterOrders()->where('status' , 'canceled')->count()}})</a>
