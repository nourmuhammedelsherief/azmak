                                        ...
                                        @foreach ($category->services as $item)
                                            ...
                                            <div class="col-md-3">
                                                <div class="image-preview" style="width: 150px; height:150px;">
                                                    <img src="{{ $item->photo == null ? asset($restaurant->image_path) : asset($item->image_path) }}"
                                                ...
                                                @php
                                                    $casher_branches = \App\Models\Branch::with('service_subscriptions')
                                                        ...
                                                        ->get();
                                                    $branchNotSubscription = App\Models\Branch::whereRestaurantId($restaurant->id)
                                                        ->whereDoesntHave('service_subscriptions', function ($d) {
                                                            $d->whereIn('service_id', [4, 9, 10]);
                                                        })
                                                        ->count();
                                                @endphp
                                                ...
                                                @if (
                                                    $myService == null or
                                                        $restaurant->branches()->whereIn('status', ['active'])->count() >
                                                            $casher_branches->count())
                                                    @if (
                                                        $myService != null and
                                                            $myService->service->id == 4 and
                                                            ($myService->status == 'active' or $myService->status == 'tentative'))
                                                    ...
                                                    @else
                                                        <a href="{{ route('restaurant.services_store.subscription', $item->id) }}"
                                                            style="margin-top: 20px;" class="btn btn-primary">
                                                            <i class="fa fa-user-edit"></i>
                                                            @lang('dashboard.subscribe')
                                                        </a>
                                                    @endif
                                                    {{-- @if ($branchNotSubscription > 0)
                                                        <a href="{{ route('restaurant.services_store.subscription', $item->id) }}"
                                                            style="margin-top: 20px;" class="btn btn-primary">
                                                            <i class="fa fa-user-edit"></i>
                                                            @lang('dashboard.subscribe')
                                                ...
                                                @elseif(
                                                    $myService != null and
                                                        in_array($myService->status, ['active', 'tentative']) and
                                                        ($item->id == 4 or $item->id == 1 or $item->id == 9 or $item->id == 10))
                                                    <a href="{{ url('restaurant/integrations') }}"
                                                        style="margin-top: 20px;" class="btn btn-primary">
                                                        <i class="fa fa-user-edit"></i>
                                                        @lang('messages.settings')
                                                    ...
                                                    @if (
                                                        $restaurant->branches()->whereIn('status', ['active', 'tentative'])->count() >
                                                            $casher_branches->count() and
                                                            $branchNotSubscription > 0)
                                                        <a href="{{ route('restaurant.services_store.subscription', $item->id) }}"
                                                            style="margin-top: 20px;" class="btn btn-primary">
                                                            <i class="fa fa-user-edit"></i>
                                                            @lang('dashboard.subscribe')
                                                ...
                                                @if (!empty($item->description))
                                                    <a href="javascript:;" data-toggle="modal" data-target="#detailsModal"
                                                        style="margin-top: 20px;" class="btn btn-info btn-details"
                                                        data-id="{{ $item->id }}"><i class="far fa-eye"></i>
            ...
            $('body').on('click', '.delete_data', function() {
                ...
                swal({
                    ...
                    window.location.href = "{{ url('/') }}" +
                        "/restaurant/res_branches/delete/" + id;
