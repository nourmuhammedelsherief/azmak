                ...
                <h3>
                    @if ($branches->count() > 0)
                        @foreach ($branches as $item)
                            <a href="{{ route('restaurant.reservation.index') }}?branch_id={{$item->id}}" class="btn btn-success" style="background-color:{{request('branch_id' ) == $item->id ? '#6a21d9' : ''}}">
                                {{ $item->name }}
                            </a>
                        @endforeach
                        <hr>
