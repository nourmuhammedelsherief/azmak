                        ...
                        <form role="form" id="post-form" action="{{ route('reservation.settings', $restaurant->id) }}"
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-12 ">
                                        <label class="control-label"> @lang('dashboard.entry.reservation_sms_otp') </label>
                                        <select name="reservation_sms_otp" id=""
                                            class="form-control select2 sms-dd"
                                            >
                                            <option
                                                value="false"{{ $restaurant->reservation_sms_otp == 'false' ? 'selected' : '' }}>
                                                {{ trans('dashboard.no') }}</option>
                                            <option value="true"
                                                {{ $restaurant->reservation_sms_otp == 'true' ? 'selected' : '' }}>
                                                {{ trans('dashboard.yes') }}</option>


                                        </select>
                                        @error('reservation_sms_otp')
                                            <p class="text-danger">{{ $message }}</p>
                                        @enderror
                                    </div>

                                    <div class="form-group col-md-12 ">
                                        <label class="control-label"> @lang('dashboard.entry.reservation_sms_success') </label>
                                        <select name="reservation_sms_success" id=""
                                        class="form-control select2 sms-dd"
                                        >
                                        <option
                                            value="false"{{ $restaurant->reservation_sms_success == 'false' ? 'selected' : '' }}>
                                            {{ trans('dashboard.no') }}</option>
                                        <option value="true"
                                            {{ $restaurant->reservation_sms_success == 'true' ? 'selected' : '' }}>
                                            {{ trans('dashboard.yes') }}</option>


                                    </select>
                                        @error('reservation_sms_success')
                                            <p class="text-danger">{{ $message }}</p>
                                        @enderror
                                    </div>

                                    <div class="sms-settings col-12">
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.sms-dd').on('change' , function(){
                var check = false;
                var items = $('.sms-dd');
                $.each(items , function(k , value){
                    var item = $(value);
                    if(item.val() == 'true'){
                        $('.sms-settings').fadeIn(400);
                        check = true;
                    }
                });
                if(!check){
                    $('.sms-settings').fadeOut(400);
                }
            });
            $('.sms-dd').trigger('change');
        });
    </script>
@endsection
