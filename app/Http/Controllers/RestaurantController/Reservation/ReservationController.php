...
class ReservationController extends Controller
{
    ...
    public function getSettings(Request $request){
        ...
        if($request->method() == 'POST'):
            ...
            $data = $request->validate([
                ...
                'reservation_sms_otp' =>  'required|in:true,false' , 'reservation_sms_success'=>  'required|in:true,false' ,
                'sms_method' => 'nullable|in:taqnyat' , 
                'sms_sender' => 'nullable|min:1|max:190' , 
                'sms_token' => 'nullable|min:1|max:190' , 
            ]);
            if($request->reservation_sms_otp == 'true' or $restaurant->reservation_sms_success == 'true'):
                $request->validate([
    ...
    public function index(Request $request)
    {
        ...
        if($request->branch_id > 0):
            $orders = $orders->whereHas('table' , function($q)use($request){
                $q->where('reservation_branch_id' , $request->branch_id);
            });
        endif;
            $orders = $orders->whereDate('date' , '>=' , Carbon::now())
            ->paginate(500);
        // return $orders;
        $status = 'active';
        $branches = ReservationBranch::where('restaurant_id' , $restaurant->id)->where('status' , 1)->get();
        return view('restaurant.reservations.orders.index' , compact('orders' , 'status' , 'branches'));
    }
    ...
    public function confirmBankOrder(Request $request , ReservationOrder $order){
        ...
        elseif($order->status == 'not_paid' and $order->payment_type == 'bank'){
            ...
            $message = 'تم انشاء طلب حجز رقم '  . $order->num;
            restaurantSendSms($order->restaurant_id, $message, $order->user->phone, 'reservation');
            flash(trans('dashboard.messages.save_successfully'))->success();
            $page = (!empty($request->page) and is_numeric($request->page)) ? $request->page : 1;
            return redirect(route('restaurant.reservation.index') . '?page=' . $page);
        }
